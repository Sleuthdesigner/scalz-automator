...
function openClientSetupWizard(onComplete) {
    ...
    function renderWizardStep() {
        ...
        if (wizardStep === 1) {
            ...
            stepDesc = 'Enter the basic details for this new client.';
            stepContent = `
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        <label for="wz-services">Services (comma-separated)</label>
                        <input type="text" id="wz-services" value="${escHtml(Array.isArray(wizardData.client.services) ? wizardData.client.services.join(', ') : (wizardData.client.services || ''))}" placeholder="Roof Replacement, Roof Repair…" />
                    </div>
                </div>
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <input type="email" id="wz-email" value="${escHtml(wizardData.client.email || '')}" placeholder="owner@acme.com" />
                    </div>
                </div>
                <div class="form-group">
                    <label for="wz-notes">Special Instructions</label>
                    <textarea id="wz-notes" rows="2" placeholder="Any special notes…">${escHtml(wizardData.client.notes || '')}</textarea>
                </div>
            `;
            ...
            stepContent = `
                ...
                <div style="margin-top:var(--space-6);padding:var(--space-4);background:var(--bg-input);border-radius:var(--radius);border:1px solid var(--border);">
                    <div style="font-weight:600;margin-bottom:var(--space-2);"><i class="fa-solid fa-circle-info" style="color:var(--blue);"></i> Setup Requirements</div>
                    <ul style="font-size:0.85rem;color:var(--text-secondary);list-style:disc;padding-left:var(--space-5);display:flex;flex-direction:column;gap:var(--space-1);">
                        <li>Scalz SEO Automator plugin must be installed &amp; activated</li>
        ...
        // Progress indicators
        const stepNames = ['Client Info', 'WordPress', 'ACF Fields', 'Review & Create'];
        const progressHtml = `
        ...
        const body = `
            ...
            <div class="modal-footer" style="margin-top:var(--space-6);">
                ${wizardStep > 1 ? '<button class="btn btn-ghost" id="wz-back-btn"><i class="fa-solid fa-arrow-left"></i> Back</button>' : '<button class="btn btn-ghost" onclick="closeModal()">Cancel</button>'}
                <div style="display:flex;gap:var(--space-2);">
                    ${wizardStep < totalSteps ? `
                        ${wizardStep === 2 || wizardStep === 3 ? '<button class="btn btn-ghost" id="wz-skip-btn">Skip <i class="fa-solid fa-forward"></i></button>' : ''}
                        <button class="btn btn-primary" id="wz-next-btn">Next <i class="fa-solid fa-arrow-right"></i></button>
                    ` : `
    ...
    function saveCurrentStepData() {
        if (wizardStep === 1) {
            ...
            wizardData.client = {
                ...
                notes: document.getElementById('wz-notes')?.value.trim() || '',
            };
        } else if (wizardStep === 2) {
            wizardData.client = {
                ...wizardData.client,
    ...
    function wireWizardEvents() {
        ...
        document.getElementById('wz-test-wp-btn')?.addEventListener('click', async () => {
            ...
            try {
                ...
                resultDiv.innerHTML = `
                    <div style="padding:var(--space-4);background:var(--red-bg);border:1px solid var(--red);border-radius:var(--radius);color:var(--red);">
                        ...
                        <div style="font-size:0.8rem;margin-top:var(--space-2);color:var(--text-muted);">
                            Make sure the Scalz SEO Automator plugin is installed and activated on the client's WordPress site.
                        </div>
                    </div>
                `;
            }
        ...
        document.getElementById('wz-create-btn')?.addEventListener('click', async () => {
            ...
            try {
                ...
                const clientPayload = {
                    ...
                    wp_app_password: wizardData.client.wp_app_password || null,
                    services: wizardData.client.services || [],
                    notes: wizardData.client.notes || null,
